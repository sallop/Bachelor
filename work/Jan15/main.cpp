...
void graph_endeffector(FILE *fp,
		       double t, VRManipulator &v1, Prismatic &r3)
{
...
void view_animation( std::string prefix )
{
  ...
  for(int ii=10; ii < static_cast<int>(Ts/dt); ii+=10){
    ...
    fprintf(gp, "splot ");
    fprintf(gp, "'dir-dat/%s_saiteki_e1_%05d.dat' w l"
	    , prefix.c_str(), ii);
    fprintf(gp, ",");
    fprintf(gp, "'dir-dat/%s_saiteki_e2_%05d.dat' w l ls 3"
	    , prefix.c_str(), ii);
    fprintf(gp, "\n");
    //    fprintf(gp, "pause 0.01\n");
    cout << __FUNCTION__<<"-"<< prefix <<"-"<< ii << endl;
...
main(int argc, char *argv[])
{
  ...
  // IDPNagato sim_nagato("nagato", 0, Ts, dt,
  // 		       kiza1, REPEAT, 8, NN);
  //   sim_nagato.Init(model, sc2, fk1, fk2, fk3,
  // 		 th2_i, th2_f, th2v_i, th2v_f, th2a_i, th2a_f);
  //   {
  //     sim_nagato.start();
  //     sim_nagato.makeGifFile();
  //   }
  //   view_animation("nagato");
  
  //IDPNagato sim_zcurve("zcurve", 0, Ts, dt,
  //kiza1, REPEAT, 8, NN);

  //IDPZCurve sim_zcurve("zcurve",
  //		       0, Ts,
  //		       dt, kiza1,
  //		       REPEAT, 8, NN);
  //sim_zcurve.Init(model, sc2, fk1, fk2,// fk3,
  //		  th2_i, th2_f, th2v_i, th2v_f, th2a_i, th2a_f);
  //{
  //  sim_zcurve.start();
  //  sim_zcurve.makeGifFile();
  //}
  //view_animation("zcurve");

  // NN => centervalue
  IDPUP sim_up("idpup", 0.0, Ts, dt, kiza1, REPEAT, 8, NN);
