...
protected:
  enum eOutputFileName
    {
      ...
      eGRAPH_ST_R3,		// 9
      eGRAPH_EV_R3,		// 10
      eGRAPH_ENDEFFECTOR,	// 11
      eENERGY,			// 12
      eENEMIN2,                 // 13 only eSaiteki l_nnn, enemin2, e1.energy, e2.energy, e3.energy, kiza
      // eOutputFileNameSize,
      // eZU_E1,			// 0
      // eZU_E2,			// 1
      // eZU_E3,			// 2
  ...
  enum eWhere{    eSinc, eSaiteki, eWhereSize    };
  enum eNameTemp{ eName, eTemp   , eNameTempSize };
  char fname_lst[eOutputFileNameSize][eWhereSize][eNameTempSize][128];
  // fname[eNameTemp][64]
  // fname[Name][64] = "Foo"
  // fname[Temp][64] = "Foo%%05d"
  
  int make_animation_flag[64];

  enum eOutputDirName { eDat, eEtc, eOutputDirNameSize, };
  char dname[eOutputDirNameSize][32];
...
class IDPUP_Mf : public IDP
{
public:
  IDPUP_Mf();
  IDPUP_Mf(string   _name,
	   double   _t_i , double _t_f  ,
	   double   _dt  , double _kiza ,
	   int	_n_repeat, int _n_divide, int _NN);
  ~IDPUP_Mf();
