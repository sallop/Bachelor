...
graph_standard(FILE *fp, double t,
	       ...
	       double energy, double ev_ia)
{
  fprintf(fp, "%8.4lf %8.4lf %8.4lf %8.4lf ", t, z, zv, za );
...
main(int argc, char *argv[])
{
  ...
  //  IDPCurve sim_curve("curve", 0, Ts, dt, kiza2, REPEAT, 8, NN);
  IDP sim_bspline("curve",
		  0, Ts, dt,
		  kiza1, kiza2, kiza3,
		  REPEAT, 8, NN);  
  {
    Bspline2d bs(pnt_i.x, pnt_i.y, pnt_i.z,
		 0.0      ,  0.02  , 0.05,
		 //0.0    , -0.05  , 0.05,
		 //0.0    , -0.10  , 0.0,
		 pnt_f.x, pnt_f.y, pnt_f.z);
    
    sc2 = SinCurv(Ts, 0.0, 1.0,  0.0, 0.0 );
    
    // sim_curve.Init(model, //sc2, fk1, fk2,
    // 		   sc2, bs,
    // 		   th2_i, th2_f, th2v_i, th2v_f, th2a_i, th2a_f);
    
    sim_bspline.Init(model, sc2, bs, pnt_i, pnt_f);
    sim_bspline.start();
    //sim_bspline.sinc();
    view_animation("curve");
    //assert(false);
    //sim_bspline.En();    
    //sim_bspline.set_position();
    //    sim_curve.start();
    //    sim_curve.makeGifFile();
  }
